...
Param* init_param(Info* info, Var* var)
{
	...
	for (i = 0; i < info->nb_layer - 1; i++)
	{
    	...
    	for (int j = 0; j < info->nb_neuron[i+1]; j++)
		{
	...
	for (size_t i = 0; i < var->train_data; i++)
	{
		param->expected_output[i] = malloc(sizeof(float) * var->train_data);
	}

	param->result = malloc(sizeof(float) * var->train_data);
	param->result = malloc(sizeof(float) * var->train_data);
