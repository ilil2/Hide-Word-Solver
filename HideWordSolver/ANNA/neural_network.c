...
int main(int argc, char** argv)
{
    if(argc > 1)
    {
        if (strcomp(argv[1], "reset"))
        {
            ...
            if (w_input == NULL || w_output == NULL || b_input == NULL || b_output == NULL) {
                errx(13, "Memory allocation error.");
                return 1;
            }
            for (int i = 0; i < input_neuron; i++) {
                if (w_input[i] == NULL) {
                    errx(13, "Memory allocation error for w_input[i].");
                    return 1;
                }
            }
            for (int i = 0; i < hidden_neuron; i++) {
                if (w_output[i] == NULL) {
                    errx(13, "Memory allocation error for w_output[i].");
                    return 1;
                }
            }
