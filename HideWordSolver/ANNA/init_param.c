...
Param* init_param(Info* info, Var* var, char state)
{
	...
	else
	{
		...
		for (i = 0; i < info->nb_layer - 1; i++)
		{
			...
			if (param->bias[i] == NULL || param->neuron[i] == NULL ||
				param->weight[i] == NULL)
			{
				err(1, "malloc()");
			}
		...
		param->result[0] = malloc(sizeof(float) * var->train_data);

		if (param->result[0] == NULL)
		{
			err(1, "malloc()");
		}
	}
